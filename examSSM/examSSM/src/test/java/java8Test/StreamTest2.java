...
public class StreamTest2 {
	...
	public static void main(String[] args) {
		...
		List<Integer> squaresList=numbers.stream()
				.map(i->i*i)//对元素进行操作
				.distinct()//去重
				.collect(Collectors.toList());//收集stream流的结果
		...
		System.out.println(IntStream.range(0, 20).max());//max或者min
		
		//获取某个范围的随机数
		for (int i=0;i<10;i++) {//输出10个两位随机数
			System.out.println("===区间rand==="+(r.nextInt(100)+1));
		}
